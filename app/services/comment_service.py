...
class CommentService:
    ...
    async def create(
        self, post_id: uuid.UUID, author: User, content: str, parent_id: uuid.UUID | None
    ) -> Comment:
        ...
        comment = await self.comments.create(
            content=content, post_id=post_id, author_id=author.id, parent_id=parent_id
        )
        await self.db.commit()
        return await self.comments.get_by_id(comment.id)

    async def list_for_post(self, post_id: uuid.UUID, offset: int, limit: int):
        items = await self.comments.list_top_level_for_post(post_id, offset, limit)
        total = await self.comments.count_top_level_for_post(post_id)
        return items, total

    async def get_by_id_or_404(self, comment_id: uuid.UUID) -> Comment:
        comment = await self.comments.get_by_id(comment_id)
        if comment is None:
            raise NotFoundError("Comment not found")
        return comment

    async def update(self, comment_id: uuid.UUID, actor: User, content: str) -> Comment:
